...
contract HolderHandler is PluginHandlerBase {
    ...
    uint256 public flushes; // coverage
    ...
    function deliverDirect(uint256 tokenSeed, uint256 payerSeed, uint256 amount) external {
        _deliverDirect(address(HOLDER), _token(tokenSeed), payerSeed, bound(amount, 0, 1e12));
    }

    function flush(uint256 tokenSeed) external {
        address token = _token(tokenSeed);
        if (HOLDER.usdcHeld(token) == 0 || MockLaunchToken(token).eligibleSupply() == 0) return;
        HOLDER.flush(token);
        flushes += 1;
    }
...
contract HolderDistributionPluginInvariantTest is Test {
    ...
    function setUp() public {
        ...
        selectors[2] = HolderHandler.flush.selector;
        selectors[3] = HolderHandler.setEligible.selector;
        targetSelector(FuzzSelector({addr: address(handler), selectors: selectors}));
        targetContract(address(handler));
    }
    ...
    function invariant_usdcHeldEqualsSumOfWaiting() public view {
        uint256 sum;
        for (uint256 i; i < tokens.length; ++i) {
            sum += holder.usdcHeld(tokens[i]);
        }
        assertEq(usdc.balanceOf(address(holder)), sum);
    }

    /// @dev Per token: waiting + distributed == credited, and the token received exactly what was distributed.
    function invariant_perTokenDistributionAccounting() public view {
        for (uint256 i; i < tokens.length; ++i) {
            address token = tokens[i];
            assertEq(holder.usdcHeld(token) + holder.totalDistributed(token), handler.ghostCredited(token));
...
contract ComboHandler is PluginHandlerBase {
    ...
    function flush(uint256 tokenSeed) external {
        address token = _token(tokenSeed);
        if (!HOLDER.isConfigured(token) || HOLDER.usdcHeld(token) == 0) return;
        if (MockLaunchToken(token).eligibleSupply() == 0) return;
        HOLDER.flush(token);
        flushes += 1;
    }
...
contract ComboPluginInvariantTest is Test {
    ...
    function setUp() public {
        ...
        selectors[4] = ComboHandler.flush.selector;
