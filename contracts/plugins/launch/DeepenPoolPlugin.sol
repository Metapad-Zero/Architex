...
///         graduation a run buys the token through the launch router with about half the offer and adds it, with the
///         rest of the USDC, to the token's launch pool, minting the LP straight to 0x…dEaD, locked forever; tokens
///         that do not fit are burned. The plugin keeps no token and no LP between calls.
///
///         Pacing is Buyback & burn's, line for line: a run offers min(held, budget), the budget being 0.25% of the
///         USDC-side reserve prorated by the time since the token's last run (a full cap for its first run), at most
///         once per token per block, never below MIN_RUN_USDC.
///
/// @dev The add (IDeepenPoolPlugin has the split formula). With the pool's reserves (T, R) before the run and
///      q = 10,000 - fee bps, a buy of b puts n = b*q/1e4 USDC in and takes t = n*T/(R+n) tokens out, leaving
///      (T - t, R + n). Those t tokens pair at that price with t*(R+n)/(T-t) = n*(R+n)/R USDC, so the add takes all of
///      them exactly when b + n + n^2/R = U, a quadratic in b whose root, rationalised to avoid cancellation, is
///      _usdcToBuy. The fees' rounding up (and the floors) leave the real add a few units off that, so the add is
///      computed afresh from the pool's reserves after the buy (the Uniswap V2 router's optimal amounts) and any
///      excess is burned (tokens) or kept for the next run (USDC). Every token bought returns to the pool, so a run
///      leaves the token reserve unchanged and raises the USDC reserve by the net buy plus the add.
///
...
///        pair and not yet synced would be folded in by the run's own buy, between the split's reading of the pool and
...
///        into a pair. The run's buy ends in the pair's swap, which sets the reserves to the balances (any earlier
///        donation is absorbed there); the run then transfers the tokens, transfers the USDC and mints, in one call,
///        with no external call in between but those two transfers (a LaunchToken and USDC, neither calls out). So
///        the mint sees exactly the run's own deposit, and the plugin checks the LP it got against its own
///        computation of the pair's formula (LiquidityMismatch).
///      - Sandwiching the add. A trader who pushes the price before a run makes the run buy and add at the pushed
///        price. But the run returns every token it buys, so the pool's token reserve, and so the trader's price
///        impact when unwinding, does not change: the trader keeps only the run's price rise (about 0.25% for a full
///        cap) against a round trip costing 2 * (0.5% + c). One run never pays, at any size, and holding through
///        runs pays only after the documented hours, about twice Buyback & burn's (half of each run buys).
...
///        the run's add.
///      - The creator fee on the run's own buy goes back to the token's plugin: straight back into this pot, or,
...
///      - Extraction: the pot leaves only through a run, into the launchpad (fees), the pair (buy and add) and the
///        token's burn. No owner, no admin, no sweep; LP only ever goes to 0x…dEaD.
contract DeepenPoolPlugin is IDeepenPoolPlugin, LaunchFeePluginBase {
        ...
        returns (uint256 usdcSpent, uint256 tokensBurned, uint256 liquidity)
    {
        ...
        Outcome memory o = graduated ? _buyInPool(token, pair, offer) : _buyOnCurve(token, offer);
    ...
    {
        if (usdcOffered == 0) return (0, 0);
        if (!LAUNCHPAD.isGraduated(token)) return (usdcOffered, 0);
        (, uint256 reserve) = _usdcSideReserve(token, true);
        usdcToBuy = _usdcToBuy(usdcOffered, reserve, _feeBps(token));
        usdcForLiquidity = usdcOffered - usdcToBuy;
    }
    ...
    /// @dev The pool run's buy and the add it implies. The add pairs every token this plugin holds (this buy's, plus
    ///      any sent to it directly) with what is left of the offer, at the pool's reserves after the buy; nothing moves
    ///      into the pair here.
    ///      The pair is synced first, so the split reads the same pool the buy will trade against: anything donated
    ///      into the pair and not yet synced would otherwise be folded into the reserves by the buy's own swap, between
    ...
    function _buyInPool(address token, address pair, uint256 offer) private returns (Outcome memory o) {
        ...
        uint256 toBuy = _usdcToBuy(offer, reserveUsdc, _feeBps(token));

        USDC.forceApprove(router, toBuy);
        o.tokensBought = ILaunchRouter(router).buy(token, toBuy, 0, address(this), block.timestamp);
        uint256 spentOnBuy = _pulledFromOffer(router, toBuy);
        if (spentOnBuy == 0) revert BadSpend(toBuy, 0);
        uint256 tokensHeld = IERC20(token).balanceOf(address(this));
        if (o.tokensBought == 0 || tokensHeld < o.tokensBought) revert NothingBought(token);
        ...
        (o.tokensAdded, o.usdcAdded, o.liquidity) = _addAmounts(
            tokensHeld, offer - spentOnBuy, reserveToken, reserveUsdcAfter, IERC20(pair).totalSupply()
        );
        o.tokensBurned = tokensHeld - o.tokensAdded;
        o.usdcSpent = spentOnBuy + o.usdcAdded;
    }
