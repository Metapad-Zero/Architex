...
///         (V13-SPEC §2.2, [D13], [D14]). Anyone can run it; each run is capped and at most one runs per token
///         per block.
/// @dev Sandwich reasoning (V13-SPEC §2.2): a run spends at most 0.25% of the USDC-side reserve, which moves the
///      price by at most ~0.5%, while a sandwich round trip pays at least ~1% in platform fees (plus the creator
///      fee twice). So a run takes no slippage bound: the cap is the protection. Each token's USDC is its own;
///      a run only ever spends the running token's balance.
contract BuybackBurnPlugin is IBuybackBurnPlugin, LaunchFeePluginBase {
    ...
    ///      launchpad, its launch router and the token being bought.
    function run(address token) external nonReentrant returns (uint256 usdcSpent, uint256 tokensBurned) {
        ...
        uint256 offer = Math.min(buyback.held, _cap(token, graduated));
        if (offer == 0) revert NothingToBuy(token);
        ...
        if (graduated) {
            ...
            (tokensBought, reportedSpend) = LAUNCHPAD.buy(token, offer, 0, address(this));
    ...
    function previewRun(address token) external view returns (uint256 usdcOffered, bool graduated) {
        ...
        usdcOffered = Math.min(buyback.held, _cap(token, graduated));
    }
