...
related_targets: ["src/components/SwapSheet.tsx","src/components/PoolsView.tsx","src/components/ConnectSheet.tsx","src/components/PoolDetail.tsx"]
---
...
- Browser wallet: there is no wallet without keys, so the sheet can make them — "Create a browser wallet" / "Import a private key" when no extension is present; the connected view for that wallet adds "Back up private key" (unlock, then reveal + copy, with the warning) and a two-step "Forget this wallet". Browser sign-in: a wallet made with a passkey IS the passkey (key derived from its WebAuthn PRF output per signature, never stored), so "Sign in with passkey" — the browser's own passkey chooser — restores it in any browser where the passkey syncs; the sheet's browser-wallet block leads with "Sign in with passkey", then "Create a browser wallet", with "Import a private key" as a quiet tertiary link. Imported keys and password wallets are stored encrypted only (passkey- or password-wrapped). Every signature goes through the confirm sheet (`UnlockSheet`, the one true modal: decoded request as receipt lines, then passkey or password), so nothing signs without the owner. The copy says so before funding.
