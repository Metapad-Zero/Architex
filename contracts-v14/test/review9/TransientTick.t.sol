...
///         Since review #9's L1 fix the bid starts from the cheaper of that tick and the graduation tick, so a buy made
///         above the graduation price shows nothing of its own tick; the unlock below therefore first sells both pools
///         under their graduation price, and most of its buys start there, where the bid must come from the buy's own
///         pre-swap tick (and one buy starts at graduation, where the cap applies).
...
abstract contract TransientTickTest is Review9Base {
    ...
    ///      from ITS pre-swap tick (capped at graduation); any other swap none. Also counts the bids placed from the
    ///      buy's own tick (under graduation), and those whose range a stale read would have changed.
        ...
        for (uint256 i; i < logs.length; ++i) {
            ...
            if (logs[i].topics[0] == BID_LOCKED) {
                ...
                if (snipe != 0) {
                    ...
                    (int24 lo, int24 hi) = _expectedRange(token, pre[s.trades]);
                    assertEq(s.lower, lo, "lower from this swap's own pre-swap tick (capped at graduation)");
                    assertEq(s.upper, hi, "upper from this swap's own pre-swap tick (capped at graduation)");
                    ++bids;
                    if (_cheaper(steps[s.trades].usdcIs0, pre[s.trades], _gradTick(token)) != _gradTick(token)) {
                        ++ownTick;
                    }
                    if (s.hasLast) {
                        (int24 staleLo,) = _expectedRange(token, s.lastPre);
    ...
    function test_everyBidInOneUnlockUsesItsOwnSwapsPreSwapTick() public {
        ...
        vm.startPrank(bob);
        IERC20(a).transfer(address(mflash), 100_000_000e18);
        IERC20(b).transfer(address(mflash), 100_000_000e18);
        vm.stopPrank();
        bool ub = _usdcIs0(b);

        MultiFlash.Step[] memory steps = new MultiFlash.Step[](12);
        address[] memory tokenOf = new address[](12);
        steps[0] = _mstep(a, _sellIn(a, 30_000_000e18)); // A under its graduation price; sells neither write nor read
        steps[1] = _mstep(a, _buyIn(a, 10_000e6)); // exact-in buy from A's own (cheaper) tick
        steps[2] = _mstep(b, _buyExactOut(b, 5_000_000e18)); // exact-out buy, other pool, from the graduation tick
        steps[3] = _mstep(b, _buyIn(b, 1_000e6)); // from above graduation: capped at the graduation tick
        steps[4] = _mstep(b, _sellIn(b, 60_000_000e18)); // B well under its graduation price, and under A
        steps[5] = _mstep(a, _buyIn(a, 10_000e6)); // back to the first pool
        // exact-out buy of 80M stopped by a price limit at B's graduation price: a partial fill from B's cheaper tick
        steps[6] = _mstep(b, SwapParams(ub, int256(80_000_000e18), TickMath.getSqrtPriceAtTick(_gradTick(b))));
        steps[7] = _mstep(b, _sellIn(b, 20_000_000e18));
        steps[8] = _mstep(b, _buyIn(b, 1_000e6));
        steps[9] = _mstep(a, _buyExactOut(a, 1e18)); // a one-token exact-out buy
        steps[10] = _mstep(a, _sellIn(a, 5_000_000e18));
        steps[11] = _mstep(a, _buyIn(a, 777e6));
        for (uint256 i; i < 12; ++i) {
            tokenOf[i] = (i >= 2 && i <= 4) || (i >= 6 && i <= 8) ? b : a;
        }
        ...
        (uint256 bids, uint256 ownTick, uint256 sensitive) = _checkEveryBid(steps, pre, logs, tokenOf);
        assertEq(bids, 8, "every buy placed its own bid (8 buys, 4 sells)");
        assertEq(ownTick, 6, "six from the buy's own tick under graduation, two from the graduation tick");
        // And the test can tell: for six of the eight buys, the previous buy's pre-swap tick (a stale read, or one from
        // the other pool) would have given a different range.
        assertEq(sensitive, 6, "a stale or crossed read would have shown");
        // Step 6 (B's fourth trade) really was a partial fill: fewer than the 80M tokens asked for.
        uint256 bTrades;
        for (uint256 i; i < logs.length; ++i) {
    ...
    function test_aSnipeFeeRoundedToZeroPlacesNothingAndTheNextBuyIsUnaffected() public {
        ...
        (uint256 placed,,) = _checkEveryBid(steps, pre, logs, tokenOf);
