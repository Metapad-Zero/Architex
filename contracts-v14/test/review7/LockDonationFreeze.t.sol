...
///         Now the hook refuses every donation, and every bid is a fresh position (its own salt) that has no fees.
abstract contract LockDonationFreezeTest is ReviewBase {
    function test_theDonationThatFrozeLockIsRefused() public {
        // A creation-block sniper pays the curve surcharge, so graduation locks a bid (the position the attack targeted).
        address token = _graduateWithCurveSnipe(0, false, dave, 2_000e6);
        assertEq(hook.bidCount(token), 1, "graduation bid");
        vm.prank(carol);
        router.buy(token, 5_000e6, 0, carol, MAX); // opening-window buy: its surcharge waits for lock()
        uint256 held = hook.lockHeld(token);
        assertGt(held, 4_000e6, "pool snipe fees held");
        ...
        // The same round trip without the donation lands, and lock() then works.
        ...
        assertLe(hook.lockHeld(token), 2, "all of it");
        assertEq(hook.bidCount(token), 2, "a second position");
