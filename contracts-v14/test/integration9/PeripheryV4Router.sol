...
// Uniswap v4-periphery 1.0.3 src/V4Router.sol (MIT, sha256 914cab05...1185), copied verbatim for the v1.4 integration
// tests. Only three things differ: the pragma (upstream pins 0.8.26; this profile compiles with 0.8.28 only), the
// relative imports (now package paths) and the contract name (PeripheryV4Router, so it cannot clash with upstream).
// The Universal Router V4_SWAP command runs this same code (its V4SwapRouter inherits V4Router). The forgefmt directives
// keep `forge fmt` from reflowing upstream's code, so it stays diffable against the original.
...
abstract contract PeripheryV4Router is IV4Router, BaseActionsRouter, DeltaResolver {
    using SafeCast for *;
