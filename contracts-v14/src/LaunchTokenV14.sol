...
/// @title LaunchTokenV14 (launchpad v1.4)
...
///           inventory), Uniswap v4's PoolManager (every pool's tokens), the launch hook, 0x…dEaD and address(0) never
///           earn.
///         - Transfers into the PoolManager revert until graduation, so nobody can open another pool for the token
///           before the launchpad opens its own at the curve's final price.
///         - `pull` lets the launchpad (curve sells) and the Architex v4 router (pool sells) move a seller's tokens
///           without an approval; each only ever passes its own `msg.sender` as the seller.
///         Everything else is v1.3's LaunchToken, line for line (V13-SPEC §3, V14-SPEC §4).
///
...
///   are never touched and they accrue nothing. The exclusion set is fixed at construction.
...
///   is about 1e11.
contract LaunchTokenV14 is ERC20, ILaunchTokenV14 {
    ...
    ///      inventory or the PoolManager, never to an arbitrary address.
    function pull(address from, address to, uint256 amount) external {
        if (msg.sender == launchpad) {
            ...
            if (to != poolManager) revert InvalidPullTarget();
        } else {
    ...
    /// @inheritdoc ILaunchTokenExtensions
    /// @dev The tracked eligible supply, which equals totalSupply() minus the excluded accounts' balances (the launchpad's,
    ///      the PoolManager's, the hook's and 0x…dEaD's),
    ///      reported as 0 below MIN_ELIGIBLE_SUPPLY, where the stream is paused.
    ...
    /// @dev Reverts any transfer into the PoolManager before graduation. Then, in this order: accrues the stream up to now
    ...
    function _update(address from, address to, uint256 value) internal override {
        if (to == poolManager && !graduated) revert PoolLockedUntilGraduation();
        _accrue();
