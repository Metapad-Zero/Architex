...
The palette is a fixed set of nine opaque inks in each print, plus one fixed black for text on yellow, plus eight stamp inks that appear only inside token marks: paper and ink establish the document, four warm grays organize hierarchy, yellow signals the next consequential action and the progress of a launch, green/red report outcomes and the direction of a market, and the stamps tell one token from another. Colour lives in the data, never in the chrome: no tinted panels, no coloured rules, no gradients. The frontmatter carries the light print; the dark print for each token is recorded beside it below and in the sidecar.

### Primary

- **Signal Yellow** (`signal-yellow`, `--accent`): The primary transaction button at rest (Connect wallet / Approve X / Swap / Add liquidity / Remove liquidity), the wrong-network wallet button as a deliberate intervention, and browser text selection (`::selection`). It also fills the launch meter (how much of a curve has sold, behind a 1px ink end-cap) and the chart's "now" marker (the latest value, ringed in ink so it holds on white paper), and it is the rule that crosses the arch in the logo. It never decorates; it marks the thing to act on or the thing being watched. It is the same yellow in both prints.
...
- **Gain Green** (`gain-green`, `--gain`; dark print #4fcf7f): Confirmed transaction lines (the check glyph, the summary text "Swapped 0.5 WETH for 1,251.20 USDC", and the explorer link that follows it); a chart series and its change readout when the window drawn ends higher than it began; the word "Buy" in a trade ledger; the "Graduated" label with its check glyph, and the full meter of a graduated launch.
- **Loss Red** (`loss-red`, `--loss`; dark print #ff6b62): A chart series and its change readout when the window drawn ends lower than it began; the word "Sell" in a trade ledger; failed transaction lines (X glyph + "Failed · reason"), over-balance amount numerals and their "Not enough X" balance line, "Not enough LP tokens", the "High price impact" receipt value above 5%, the connect sheet's form and unlock errors ("A private key is 64 hex characters, with or without 0x.", "Use at least 8 characters.", "The two passwords differ.", "Passkey request cancelled.", "That password did not unlock the wallet.") and the confirm sheet's ("That password did not unlock the wallet.", "Passkey request cancelled. Try again or cancel below."), each a 14px sentence with `role="alert"` under its field, and the label of the one destructive confirm ghost, "Forget for good".

### Identity

- **Stamp inks** (`--stamp-0` … `--stamp-7`, with `--on-stamp` for the initials): eight spot colours used only as the fill of a 24px token mark. Light print, white initials: cobalt #1f47d1, violet #6a3bc0, magenta #b8206f, orange #c2500a, teal #0a756e, sky #1a6fa8, olive #5f6b10, umber #87481f. Dark print, black initials: #7d9bff, #b79aff, #ff8ac4, #ff9d52, #4fd1c5, #6cc4ff, #c5d34a, #e0a476. Every pairing clears 4.5:1 for the initials and 3:1 against paper. None is Signal Yellow, Gain Green or Loss Red, so a stamp is never mistaken for a signal. The canonical tokens keep a fixed stamp near their own colour (USDC cobalt, WETH violet, WBTC orange, ARC teal, EURC sky; canonical by address, so a launch that calls itself USDC does not get cobalt); every other token takes its stamp from its address (`tokenStamp` in `src/lib/tokens.ts`), so it wears the same colour in every list and on every visit. A token image, when a launch supplies one, is laid over the stamp once it has loaded; a slow or broken image never leaves a hole. A pool shows its two stamps side by side (`PairMarks`). The classes are built at run time (`stamp-${n}`), so their rules live outside Tailwind's purged layers.
...
- **Warm Gray 500** (`warm-gray-500`, `--g500`; dark print #9c9c98): Field labels ("You pay"), balance and USD lines, receipt keys, table headings, section counts, placeholders, the ghost "…" and ghost zeros, empty-state and helper copy, disabled text, the network name in the masthead, the step hint under the primary button, the "Connected wallet" / "Browser wallet" labels, the "Detected" status, the browser wallet's short address in its row, the "Private key", "Unlock with", "Wallet password" and "Repeat password" labels in the connect sheet and the confirm sheet's "Signing as 0x7212…1D46 · browser wallet" footer, ledger timestamps, the price-history heading and axis text, and the loading state of the price series and markers.
- **Warm Gray 700** (`warm-gray-700`, `--g700`; dark print #c9c9c5): Explanatory messages that need more weight than metadata: the quote message under the receipt ("Not enough liquidity in this pool for that amount."), the pending and cancelled transaction lines, the "You are creating this pool" note, and connect-sheet messages ("No wallet extension found…", "Request rejected in your wallet."), including the browser wallet's custody sentences, the protection sentence under its create/import form, the confirm sheet's note and passkey sentence, the "Never share this private key." warning, the "Forget this wallet" text link and its confirm sentence.

### Named Rules

**The Nine-Ink Rule.** Use the fixed opaque palette; do not create alpha tints, near-duplicate grays, or decorative colors. State changes are ink swaps (paper → gray 100 → ink), never opacity changes. The document is printed in exactly nine inks per print; the dark print is a value swap on the same nine tokens. The eight stamp inks are not part of the document: they fill token marks and nothing else, never text, a rule, a button or a surface.

**The Signal Rule.** Signal Yellow means one of four things and nothing else: the money-moving action, a wrong-network intervention, progress toward graduation (the launch meter), or where a value is now (the chart marker); text selection is the fifth, borrowed by the browser. A screen carries one yellow button at most. A list of launches carries one meter per row, which is the point of the list; a meter is 8px tall and never grows into a panel.
...
**The Direction-Ink Chart Rule.** A chart draws one series. It is Gain Green when the window drawn ends above where it began, Loss Red when it ends below, and Ink when it is flat; the head says the same thing first with an arrow glyph and a signed percentage, so the ink only repeats it. Markers and the crosshair are Ink with a paper ring, the latest point is the Signal Yellow "now" marker ringed in Ink, the grid is Warm Gray 300, axis text is Warm Gray 500, and loading dims the series to Warm Gray 500. There is no categorical palette, no fill under the line and no second series. The same component (`PriceHistory`) draws pool prices under the swap sheet and in a pool's detail, and market cap on a launch's page.
...
**The Fixed-Slot Rule.** Every number is tabular; empty amount fields show a ghost "0" in Warm Gray 500; the receipt block always renders its five rows (Rate / Price impact / Fee / Minimum received / Route) with the ghost "…" (`GHOST` in `src/lib/format.ts`) until a quote exists, so nothing counts, jitters or reflows while the user types. A receipt row is one line at every width: keys never wrap (`dt` nowrap), values never wrap and clip with an ellipsis (`dd` nowrap + ellipsis), and a value that would not fit below 640px is written in its short form ("9.06% · High impact · 10% of pool") rather than allowed to grow the row.
...
The interface follows an 8px module (`--unit`) with 4px half-steps where dense rows or mobile controls need them; observed gaps are 4, 8, 12, 16, 20, 24, 28, 32, 40, 48 and 64px. The masthead is a 56px band closed by a black hairline, with 16px side padding rising to 24px from 640px. The Swap task is a single centered column capped at 512px including 16px gutters (a 480px sheet), starting 48px below the masthead on compact screens and 64px from 640px, with 96px of bottom padding. The recent ledger sits 56px beneath the sheet as its own ruled section. The Pools task widens the same grammar to 1008px with 16px / 24px gutters and 40px between the page title and the first section.
...
A launch may carry details beyond its name and symbol: an image, a description of up to 280 characters, and links to a website, X and Telegram (`docs/launchpad/METADATA.md`). They are the creator's words, so they are set apart and signed as such. On a token's page they sit between the header and the chart in a `launch-about` block: Warm Gray 300 hairlines above and below, 20px vertical padding, capped at 672px, 14px text on a 24px line; the description keeps its line breaks; links run in a wrapping row, each reading "Website · example.com" with the external-link glyph, where the second half is always the real host or handle, never text the creator chose (an internationalised host shows in punycode, so a look-alike shows as what it is); the block closes with the 12px Warm Gray 500 line "Written by the creator. Architex has not checked it." The token's image is laid over its stamp (24px in lists, 48px at the head of its own page, `token-mark-lg`) once its bytes have been verified against their address; a row asks for its details only when it first scrolls into view.

On the create form the same details are a `launch-details` fieldset under a Warm Gray 300 hairline, headed "Details" (14px semibold) with the 12px note "All optional. They are stored on IPFS and cannot be changed after launch." The image control is a ghost button ("Choose image" / "Change image") beside a 44px preview, the file's name in Warm Gray 500 and an underlined "Remove"; its hint says what happens to the file ("PNG, JPEG, WebP or GIF. Resized to 512px; location data in the photo is removed."). The description is a textarea inside the compact-field frame, which grows with it, with a live "55/280" counter in the suffix slot. Errors follow the form's rule: a 14px Loss Red sentence under the field, shown after the first submit. While details are being pinned the primary button reads "Saving details…" in its loading state; this happens on the press that creates the token, never on the approval press. The fieldset is absent until the server reports that a pinning key is configured.

### Price History

The price history (`price-history`, component `PriceHistory`) is a single-series line that takes a plain value series, so one component draws three things: a pool's price in its detail row, the price of the pair being swapped under the swap sheet (`SwapPriceChart`, loaded lazily and drawn only when the two tokens share a funded pool; quoted in USDC when USDC is one side, otherwise in the token being paid), and a launch's market cap at the head of its page (from the virtual reserves each `Trade` event carries, with the creation point drawn first only when every trade since is known). The series follows the Direction-Ink Chart Rule; after the current reading the head carries a 16px trend arrow and a signed percentage in the same ink while no point is hovered; the latest point is always drawn as the r=5 Signal Yellow "now" marker. A head row opened by an ink hairline (8px top padding, 32px minimum, baseline-aligned, wrapping) carries "Price history · WETH per WBTC" in 14px Warm Gray 500 on the left and the current reading on the right (the price in 14px semibold, the timestamp 8px after it in Warm Gray 500); the reading follows the crosshair and falls back to the latest point. The plot is an inline SVG 168px tall at the column's width (padding 12 / 12 / 28 / 56px), `role="img"` with a spoken label, `tabIndex=0`, `touch-action: none`, non-selectable: at most three Warm Gray 300 1px grid lines with 12px tabular Warm Gray 500 tick labels right-aligned 8px left of the plot; first and last timestamps at the baseline; the series as a 2px Ink stroke with round joins and caps and no fill; when the series has 24 points or fewer, r=4 Ink markers with a 2px Paper ring at every point; on hover, pointer-down or arrow keys, a 1px Ink crosshair spanning the plot height with an r=5 active marker. Left/Right arrows step the reading; Escape and blur clear it; the crosshair is pointer-following, not animated. Loading dims the series and markers to Warm Gray 500 behind the same head. "Reading the pool's history…" and "No trades yet. The first swap starts the price history." are the 14px Warm Gray 500 sentences shown in place of the plot (24px vertical padding); when only the recent end of the history could be read the sentence is "No recent swaps. Older price history could not be loaded.", because an empty chart must never claim there were no trades. History comes from the explorer's v2 log API with incremental polls (`src/lib/explorerLogs.ts`), falling back to three RPC windows (`src/lib/rpcLogs.ts`). A right-aligned underlined "Show as table" / "Hide table" (14px, 8px above) toggles a `price-table` under the plot: 14px, 12px top margin, regular Warm Gray 500 headings (Time / Block / unit) on an ink rule, Warm Gray 300 row rules, numeric columns right-aligned, newest first, capped at 50 rows.
...
- **Don't** put an em dash in anything the site shows, copy or empty slot: a sentence takes a comma, colon, period or parentheses instead (never a hyphen or an en dash in its place), and an empty slot takes the ghost "…" (`GHOST` in `src/lib/format.ts`). `src/lib/__tests__/noEmDash.test.ts` fails on one.
